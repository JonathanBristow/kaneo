...
- Agent member keys, which give automation its own workspace identity instead of a person's
...
An [agent member](/core/functional/agent-members) is a non-human workspace member with its
own name and its own API key. Requests made with an agent key are attributed to the agent
rather than to a teammate, which keeps automated task and comment history distinguishable
from human activity. Use one when a bot, script, or AI agent should act as itself in a
single workspace.

Agent keys are created by a workspace Owner or Admin from the **Members** page, are shown
once, and expire after 90 days.
...
<Note>
The steps above create a **personal** API key that acts as you. To create a key that acts
as a dedicated automation identity instead, see [Add agent
members](/core/functional/agent-members).
</Note>
...
- **Give automation its own identity**: Use an agent member key for bots and services rather than a personal key, so access can be revoked without affecting a person's account
